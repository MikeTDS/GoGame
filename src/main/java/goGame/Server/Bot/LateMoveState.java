...
class LateMoveState extends MoveState {
    ...
    public void setPointsSystem() {
        ...
        _pointsMap.put("territoryExpansion", 400);
        ...
        _pointsMap.put("territoryShrink", -800);
    }
    ...
    public int findBestField(Bot player){
        ...
        if(checkIfShouldPass())
            return -1;

        return chooseBestField();
    }

    private boolean checkIfShouldPass() {
        for(int point : _pointsBoard)
            if(point > 0)
                return false;
